...
bool remove_by_name(char name_to_remove[NAME_MAX])
{
    // Check if the list is empty
    if (pHead == NULL)
    {
...
bool remove_by_priority(int priority)
{
    //check if its empty
    if (pHead==NULL)
    {
    ...
    //find if the priority given exists in the list 
...
int main()
{
    ...
    add_to_back((char*)"Derick",13);
    ShowYourself();

    add_to_front((char*)"Lucian",4);
    ShowYourself();

    remove_by_name((char*)"Derick");
    ShowYourself();
